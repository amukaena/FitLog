...
| 2026-03-10 | 3차 개발: 운동별 진행 차트(통계) 화면 추가 |
...
## 10. 3차 개발: 운동별 진행 차트 (2026-03-10) ✅

### 개요
특정 운동의 중량/볼륨 변화를 시간순 라인 차트로 시각화하여 성장 추이를 확인하는 기능.

### 진입 경로
- 캘린더 화면 → 상단 AppBar "통계" 아이콘(BarChart) → ExerciseStatsScreen

### 생성된 파일
```
data/local/dao/StatsDao.kt                    -- 통계 전용 DAO (집계 쿼리)
presentation/stats/ExerciseStatsScreen.kt      -- 통계 화면 UI (Canvas 라인 차트)
presentation/stats/ExerciseStatsViewModel.kt   -- 통계 상태 관리
```

### 수정된 파일
```
data/local/FitLogDatabase.kt                   -- StatsDao 등록
di/AppModule.kt                                -- StatsDao DI 제공
presentation/navigation/Screen.kt              -- ExerciseStats 라우트 추가
presentation/navigation/NavGraph.kt            -- ExerciseStats composable 추가
presentation/calendar/CalendarScreen.kt        -- 통계 아이콘 버튼 추가
```

### 주요 기능
- 부위별 카테고리 필터 (전체/가슴/등/어깨/팔/하체/코어)
- 운동 선택 드롭다운 (필터된 운동 목록)
- 기간 필터: 4주 / 8주(기본) / 12주 / 전체
- 차트 탭 전환: 최고 중량 / 총 볼륨
- Compose Canvas 기반 라인 차트 (외부 라이브러리 없음)
- 기록 요약: 역대 최고 중량(날짜), 최근 중량, 총 수행 일수, 최초 기록 날짜

### 기술 노트
- DB 마이그레이션 불필요 (기존 테이블 집계 쿼리만 사용)
- 외부 차트 라이브러리 없이 Compose Canvas로 직접 구현
- StatsDao는 향후 기능 2(PR 감지), 기능 3(월간 요약)에서도 공유 가능
...
### 3차 개발
- [x] 운동별 진행 차트 (최고 중량/총 볼륨 라인 차트, 기간 필터, 기록 요약)
- [x] 운동 통계 화면 부위별 필터링 (가슴/등/어깨/팔/하체/코어 카테고리 필터)
...
- [ ] UI 개선 및 애니메이션
- [x] ~~운동 통계 화면~~ → 10절에서 구현 완료
- [ ] 알림 기능
- [ ] 개인 기록(PR) 자동 감지 (10번 문서 기능 2)
- [ ] 주간/월간 요약 통계 (10번 문서 기능 3)
